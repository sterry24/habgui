...
#from pkg_resources import resource_filename ##  may not need
import misc
from gateway import *
...
class HabGui:
	def __init__(self):
		...
		# Read config file
		self.ConfigFileName = os.path.join(os.environ['HOME'], '.config/habgui.ini')
	...
	def _UpdateGPSPosition(self):
		...
		return True     # So we don't get called again, until there's a new GPS position
	...
	def LoadSettingsFromFile(self, FileName):
		...
		self.GPSDevice = config.get('GPS', 'Device', fallback='/dev/ttyUSB0')
	...
	## this may change when setting up tracker
...
## First make sure that gpsd is running with NMEA
#subprocess.check_call(["sudo","gpsctl","-n"])
#print("Starting GUI")
